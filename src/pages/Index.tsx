...
const Index = () => {
  ...
  // Function to create conversation summary for context
  const createConversationSummary = (messages: Message[]): string => {
    if (messages.length <= 5) return '';
    
    const recentMessages = messages.slice(-5);
    const summary = recentMessages.map(msg => 
      `${msg.isUser ? 'User' : 'Assistant'}: ${msg.text.substring(0, 100)}${msg.text.length > 100 ? '...' : ''}`
    ).join('\n');
...
- Make responses easy to read and understand

7. Response Guidelines:
- Provide complete, comprehensive answers to user questions
- Don't artificially limit response length - give full information
- Use clear structure with paragraphs, bullet points, and formatting
- If the response becomes long, that's fine - it will be automatically formatted for readability

8. Conversation Context Guidelines:
- ALWAYS maintain conversation context and flow
- If a user answers "yes" or "no" to your question, continue from that context
- Don't ask new questions unless the current topic is complete
- Build upon previous responses and user inputs
- If you asked a yes/no question and got an answer, provide the next logical step
      ...
      const response = await fetch('https://api.mistral.ai/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            // Include conversation summary for long conversations
            ...(messages.length > 10 ? [{ 
              role: 'system', 
              content: createConversationSummary(messages) 
            }] : []),
            // Include recent conversation history for context (last 8 messages to stay within token limits)
    ...
    <div className={`min-h-screen flex flex-col bg-gradient-to-br from-[${currentTheme.colors.background.from}] via-[${currentTheme.colors.background.via}] to-[${currentTheme.colors.background.to}]`}>
      ...
      <div className="flex-1 max-w-4xl mx-auto px-4 py-6 mb-16">
        <Card className={`bg-white/90 backdrop-blur-sm border-[${currentTheme.colors.primary.to}]/20 shadow-xl`}>
          {/* Messages */}
          <div className="h-[60vh] overflow-y-auto p-6 space-y-4 chat-container smooth-scroll">
